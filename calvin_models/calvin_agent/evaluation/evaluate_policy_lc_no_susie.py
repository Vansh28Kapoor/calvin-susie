...
import lc_diffusion_policy_no_susie
...
os.environ['XLA_PYTHON_CLIENT_PREALLOCATE'] = 'false' #JAX starts with minimal GPU memory and allocates more as needed (dynamic allocation) instead of preallocating all GPU memory
os.environ['XLA_PYTHON_CLIENT_MEM_FRACTION'] = '0.7' #JAX can use up to 70% of total GPU memory
...
class ZeroGoalLanguageConditionedModel(CalvinBaseModel):
    """Custom model that uses language-conditioned diffusion policy without SUSIE subgoal generation.
    """
    
    def __init__(self):
        self.lc_policy = lc_diffusion_policy_no_susie.LCDiffusionPolicyNoSUSIE()
        self.log_dir = "experiments_lc_no_susie"
    ...
    def step(self, obs, goal):
        ...
        # Query the language-conditioned behavior cloning model (no goal image concatenation)
        action_cmd = self.lc_policy.predict_action(rgb_obs, self.language_task)
...
def main():
    ...
    if args.custom_model:
        model = ZeroGoalLanguageConditionedModel()
