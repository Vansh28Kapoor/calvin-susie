...
class LCDiffusionPolicy:
    """Language-Conditioned Diffusion Policy for CALVIN evaluation.
    
    This policy uses the lc_ddpm_bc agent (language-conditioned goal-conditioned DDPM BC)
    which directly predicts actions from language instructions without needing 
    ...
    def __init__(self):
        ...
        train_paths = [[
            "mini_dataset_susie/susieprocessed_tfrecords_4step_training_A_traj0.tfrecord",
            "mini_dataset_susie/susieprocessed_tfrecords_4step_training_A_traj1.tfrecord"
        ]]
        ...
        dataset_kwargs = dict(
            ...
            act_pred_horizon=4,
            obs_horizon=1,
            load_susie_goal_images=True,  # This is the key difference
        )
        ...
        # Use the FiLM encoder for language conditioning
        encoder_def = encoders["resnetv1-34-bridge-film"](
            pooling_method="avg",
        ...
        if resume_path is None:
            raise ValueError("LC_POLICY_CHECKPOINT environment variable not set")
    ...
    def predict_action(self, image_obs: np.ndarray, goal_image: np.ndarray, language_instruction: str):
        """Predict action from concatenated obs+goal and language instruction.
        
        The key insight from calvin_dataset.py:
        - SUSIE goal images are CONCATENATED with observations along channel dimension
        - obs shape: (200, 200, 3) + goal shape: (200, 200, 3) = (200, 200, 6)
        
        Args:
            image_obs: RGB image observation (H, W, 3)
            goal_image: Generated goal image from diffusion model (H, W, 3)
            language_instruction: Natural language instruction string
            
        Returns:
            action: Predicted 7-DOF action (x, y, z, rx, ry, rz, gripper)
        """
        # Concatenate goal image with observation along channel dimension
        # This matches how the training data was prepared in calvin_dataset.py
        concatenated_obs = np.concatenate([image_obs, goal_image], axis=-1)
        assert concatenated_obs.shape[-1] == 6, f"Expected 6 channels, got {concatenated_obs.shape[-1]}"
        # breakpoint()
        ...
        # Sample actions from the language-conditioned policy with concatenated obs+goal
        action = self.agent.sample_actions(
            {"image": concatenated_obs[np.newaxis, ...]}, 
